...
- Every quality gate (`test`, `rubocop`, `flog_check`, `flay_check`, `reek`, `typos_check`, `fasterer_check`) now writes its full detailed output to a `<gate>_output.txt` file at the repo root (gitignored via `*_output.txt`) and prints only a one-line summary to stdout — full detail stays on disk without cluttering the terminal.
